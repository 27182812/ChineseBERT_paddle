...
from pdchinesebert import ChineseBertForQuestionAnswering, ChineseBertTokenizer
...
def evaluate(model, data_loader, args, output_dir="./"):
    ...
    eval_results = get_result(ground_truth_file=ground_truth_file,prediction_file=os.path.join(output_dir, "all_predictions.json"))
...
def main(args):
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(
                os.path.join(os.path.dirname(args.output_dir), "train_cmrc2018.log"),
                mode="w",
                encoding="utf-8",
            )
        ],
    ...
    for _ in range(args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with auto_cast(
                ...
                logits = model(input_ids, token_type_ids=token_type_ids,pinyin_ids=pinyin_ids)
                loss = (
...
def parse_args():
    ...
    parser.add_argument("--data_dir", type=str,default="data/cmrc2018", help="the path of cmrc2018 data.")
    parser.add_argument("--output_dir",default="outputs",type=str,help="The output directory where the model predictions and checkpoints will be written. ""Default as `outputs`")
    ...
    parser.add_argument("--eval_batch_size",default=16,type=int,help="Batch size per GPU/CPU for evaluating.")
    ...
    parser.add_argument("--max_train_steps",default=-1,type=int,help="If > 0: set total number of training steps to perform. Override num_train_epochs.")
    parser.add_argument("--warmup_radio",default=0.1,type=float,help="Proportion of training steps to perform linear learning rate warmup for.")
    ...
    parser.add_argument("--writer_type",choices=["visualdl", "tensorboard"],default="visualdl",help="writer_type.")
    parser.add_argument("--device",choices=["cpu", "gpu"],default="gpu",help="Select which device to train model, defaults to gpu.")
    parser.add_argument("--scheduler_type",choices=["linear", "cosine", "poly"],default="linear",type=str,help="scheduler_type.")
    ...
    parser.add_argument("--n_best_size",type=int,default=35,help="The total number of n-best predictions to generate in the nbest_predictions.json output file.")
